...
if __name__ == "__main__":
    ...
    if not args.leave_recombinant_mutations:
        ...
        unique_pairs = np.unique(
            [
                tables.edges.parent[re_node_desc_edges],
                tables.edges.child[re_node_desc_edges],
            ],
            axis=1,
        )
        parent_id, children_per_parent = np.unique(
            unique_pairs[0, :], return_counts=True
        )
        re_nodes_with_one_child = parent_id[children_per_parent == 1]
        assert np.all(
            (tables.nodes.flags[re_nodes_with_one_child] & sc2ts.NODE_IS_RECOMBINANT)
            != 0
        )
