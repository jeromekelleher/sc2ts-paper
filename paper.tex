...
ancestral mutation (see Section~\ref{sec:args} for precise definitions). \sally{[What confuses me is that I would think that none of these reversions would remain after the reversion push step of Figure 1D.  Is this because ``immediately ancestral'' is incorrect or because the problem is that some of these immediate reversions are created deeper in the tree than the newly added samples (i.e., uncorrected by the reversion push). If the former, maybe say ``reverting the state change of a recent ancestral mutation'', if the latter, maybe say  ``(see Section~\ref{sec:args} for precise definitions, note that the reversion push algorithm of Figure 1D only eliminates reversions in the newly added portion of the tree and not reversions around earlier nodes created by this algorithm)''.]}
...
are similar.
Here we define the
interval \sally{within which a particular breakpoint may have occurred} as the genome coordinates over which the
...
somewhat arbitrary, because they are defined by \sally{the sequence differences that happen to be present in the} recombinant's parents.
...
and \sally{potentially} should be included in the list of ``problematic sites'' that
...
\sally{[Delete this paragraph now? The new section 4.8 gets at the real issue, that the multi-step mutations common in new VoC generate false positive signals of recombination.]} The most recent recombination node for 79 samples (XN: 53, XZ: 16, XAJ: 6, XE:
1, XAD: 1, XAH: 1, XAK: 1) is the same. This node, introduced as a
result of adding strain
Denmark/DCGC-281594/2021 (BA.2.9), is ancestral to more than 127K samples, 
and is highly likely to be a spuriously inferred recombination event. \sally{[Replace with ``...is ancestral to more than 127K samples and XX immediate descendants. While such a hyper-connected node may reflect a large outbreak with little phylogenetic resolution, it may also be spurious, e.g., caused by the attraction of sequences with similar constellations of mutations or another phylogenetic artefact.'']}
Understanding the origin of this
and other similar high-impact, but potentially spurious, recombination
...
ancestral to a large number of samples\sally{. These inferences of recombination may well be artefacts caused by the appearance of new variants of concern carrying more than the expected number of mutations \cite{otto2021origins}}. Table~\ref{tab:false_positive} shows details for four recombinants
...
labelled A--D. 
\sally{[DELETED SENTENCE]}%These false positives are associated with the origins of large outbreaks. 
...
is a signature feature of SARS-CoV-2~\citep{Corey2021-sars,otto2021origins,Nielsen2023-host}.
...
ratio). The first few sequences from these new lineages will \sally{be a poor match} to
the existing ARG, and the HMM will therefore
search for ways to reduce the number of mutations required by recombining
segments. 
\sally{[DELETED SENTENCE (TRUE WHETHER OR NOT QUALITY ISSUES OCCUR).] }%If this is combined with data quality issues and ``time travellers'' (Section~\ref{sec:filtering_time_travellers}), the early stages of the VoC emergence can be quite muddled.
...
Emissions may also be a function of the nucleotide states, but in our RNA virus case, we assume that \sally{mutations occur to all possible alternate alleles} ($A,C,G,U,-$) with equal probability $\mu_\ell/5$. \sally{[Why is this 5?  I wouldn't have counted mutations to the same allele as a mutation.]} This is reasonable for rapidly evolving pathogens, but we note that setting the number of alleles at site $\ell$ ($a_\ell$) to the set of observed alleles across all analysed samples ($\mu_\ell/(a_\ell-1)$) will often be more appropriate
...
where \sally{$\alpha = (1 - r(n-1)/n) $ represents the probability of a sequence not recombining at a given position with any of the other $n-1$ members of the reference panel [REVISION OK?]} and $c$ is the
...
\sally{recombination is increasingly favoured over multiple mutations in a specific ancestral genome}.
%We use an MMR value of $k=3$ in this work, as it provided results that tallied well with empirical observations.
\sally{We use an MMR value of $k=3$ in this work,
because of the relatively high rate of recombination relative to mutation typical of coronaviruses \cite{amoutzias2022remarkable}.}
...
to any other state as 1. \sally{[Repeated clause in the last sentence?  Also, I'm a bit unclear if this is enough.  Let's say that the nodes were G->Unknown->G.  The latter is given a probability of one, but is the former G->Unknown also allowed with probability one?  Note, however, that G->G shouldn't occur with probability one but Exp[-mu/t]]}
