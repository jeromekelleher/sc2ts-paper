...
\sally{[What confuses me is that I would think that none of these reversions would remain after the reversion push step of Figure 1D.  Is this because ``immediately ancestral'' is incorrect or because the problem is that some of these immediate reversions are created deeper in the tree than the newly added samples (i.e., uncorrected by the reversion push). If the former, maybe say ``reverting the state change of a recent ancestral mutation'', if the latter, maybe say  ``(see Section~\ref{sec:args} for precise definitions, note that the reversion push algorithm of Figure 1D only eliminates reversions in the newly added portion of the tree and not reversions around earlier nodes created by this algorithm)''.]}
