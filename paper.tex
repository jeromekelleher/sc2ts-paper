...
are the biases?]
% This resource shows the typical workflow for manually identifying recombinants that the pango team uses: https://pha4ge.org/resource/identifying-sars-cov-2-recombinants/
% Regarding bias, not peer-reviewed but in the official pango designation post for XAC, you can see that me (Katherine) and the authors had a multi-month long discussion on how many breakpoints there should be and where. Reproducibility was difficult: https://github.com/cov-lineages/pango-designation/issues/590
...
Li and Stephens (LS) model (Figure~\ref{fig:overview_sc2ts}A, B; Section~\ref{sec:ls}). Then, since many samples
in the batch can share an attachment path,
we infer phylogenetic trees for each of the clusters separately using standard
methods (Figure~\ref{fig:overview_sc2ts}C; Section~\ref{sec:sample-cluster-tree-inference}).
...
topology (``UPGMA" indicates a node inferred to be an ancestor connecting a sample to the existing tree). Similarly, edges are classified by the type of the parent node.
...
Both ARGs contain 29,422 sites, due to the large number of sequences (Section \ref{sec:data_preprocessing}),
...
mutations per sampled genome, demonstrating that most added samples fit into the ARG parsimoniously. See Section~\ref{sec:mutation_spectrum}
...
near the tips (e.g.\ cross-crossing assingments within the Alpha clade). It is unclear what
...
These findings are closely replicated in both the Wide and Long ARGs (Table \ref{tab:jackson}).
...
from parents that have diverged only a handful of days ago, to much more
...
% Anyone reading this will know what these are
% (B.1: original variant, B.1.617.2: Delta, BA.1: first Omicron wave,
% and BA.2: second Omicron wave)
% as demonstrated by the number of samples with those
% Pango designations traback to each ancestral node.
% The majority of these frequently-shared MRCAs
...
non-sample nodes (i.e. inserted by sc2ts, see Sections~\ref{sec:sample-cluster-tree-inference} and \ref{sec:parsimony-heuristics}). Dotted lines show where this subgraph links
...
https://gisaid.org/; access on March 27, 2023). High prevalence increases the risk of coinfection, providing opportunities for new phenotypically
distinct recombinants to emerge and spread. Phylogenetic approaches have
been central to pandemic responses thus far \citep{Attwood2022-ab}.
% 1. Form the basis of variant classifications (UShER paper),
% 2. Global context. In 2020, Nextstrain provided situation reports that were interactive, phylogenetically-driven visualizations of SARS-CoV-2 characteristics such as transmission https://nextstrain.org/ncov-sit-reps/
% 3. The WHO's recommendations on Omicron asked countries to "report initial cases/clusters associated with VOC infection to WHO". Phylogenetics being one way to identify those crucial early clusters. https://www.who.int/news/item/26-11-2021-classification-of-omicron-(b.1.1.529)-sars-cov-2-variant-of-concern
However, with the rise to high frequency of recombinant lineages \citep[e.g. XBB; ][]{Tamura2023-ab},
...
% Citation: https://doi.org/10.1007/s00705-020-04750-z . This is the data source we used to create the breakpoints motif look up table.
...
[FIXME/TODO - include a reference to the GISAID EPI SET pdf]
