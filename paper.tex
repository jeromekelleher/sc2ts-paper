...
interval for a particular breakpoint as the genome coordinates over which the
...
(Section~\ref{sec:ls}) and the left bound is then derived by
...
better. ] Firstly, in several cases there is a clear relationship between
...
It is important to note that the exact endpoints of intervals can be
somewhat arbitrary, since they are defined by exact comparisons
of a recombinant's parents. The presence of problematic sites that are enriched
...
region) has the largest number of mutatations and corresponds to the
...
simple-origin (XA) type with no other Pango designations among their descendants.
The remaining 10 are of the XAG type with multiple Pango
designated lineages as additional descendants of the originating recombination event. In some cases,
these may be, however, be a result of erroneous Pango designations. Supplementary Table~\ref{tab:pango-recombinants} also shows the official Pango designated parent lineages for
each recombinant lineage. The parent lineages in the Long ARG agree to a great extent with these, although \texttt{sc2ts} provides more a precise
parent designation.
...
recombination node in the ARG. Of these, 11 are cases where none of the
recombinants are nested (i.e. there is no overlap between the descendant samples
of each recombination node). In 9 of these 11, there is a single ``main''
recombination event from which the majority of the corresponding recombinant samples
descend (in other two, there are too few Pango X samples to decide on a ``main'' clade:
...
XN lineage (originally defined on the basis of sequences from the UK). The
Long ARG infers four independent origins for nonsingleton Nextclade-designated XN
...
(marked with asterisks in Supplementary Table~\ref{tab:pango-single-origin}; the most
...
XN samples are grouped into rows based on their most recent ancestral recombination node;
Details are also provided for the ``causal strain'' that led to the recombination
being proposed by \texttt{sc2ts}. Note that a large number of XN samples were previously removed by filtering. When undersampling causes three or more mutations to lie immediately above the causal strain, \texttt{sc2ts} may mistakenly propose multiple independent origins rather than one.
}
\end{table}
