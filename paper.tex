...
we demonstrate that \emph{sc2ts} can be used to simultaneously (1) detect putative SARS-CoV-2
...
\emph{Sc2ts} is a promising method to automate detection of SARS-CoV-2 recombinants
...
Although recombination nodes are relatively infrequent, the wide and long ARGs still
contain many of them (4123 and 2078 respectively; Supplementary Tables 1 and 2).
Each of these nodes were
...
Nextclade-designated Pango lineages, trace back to 50 different recombination nodes.
...
Nextclade-identified XA samples (in red) to the closest other sample nodes in our dataset. The
subgraph may thus include inserted, non-sample nodes (i.e. UPGMA, recombination, mutation
collapse, or reversion push nodes, all plotted as smaller circles) on the path to the nearest
sample nodes (larger circles). Dotted lines show where this subgraph links to the rest of the
ARG. Above recombination nodes only ancestral nodes are shown, so that e.g. cousins of non-sample
recombinant parents are not shown. It is clear from the subgraph that all the XA samples trace
to a single originating
recombination node (plotted in black), with a breakpoint position around 22227bp.
...
all trace back to the same most recent common recombination node (black), but
...
as BA.2 (and XAG becomes fully monophyletic, see supplementary figure
\ref{fig:pango_XAG_gisaid_graph}).

In total we identify 16/31 single-origin Pango recombinant lineages in the long ARG.
Supplementary Table~\ref{tab:pango-single-origin} summarises these origination events
...
these may be caused by samples which have been designated as the wrong recombinant lineage
by Nextclade.
...
The different geographical locations, the time between the samples, and
the fact that the two samples differ at 23 nucleotide positions,
suggests that these may indeed represent true independent origins of XD.
...
Another X lineage we infer to have multiple origins is XE. In this case, 
we identify at least 4 independent origination events of the Nextclade-designated
XE samples (Supplementary Table~\ref{tab:multiple_origins_table}).   
...
\begin{tabular}{lllrl}
\toprule
 & Sc2ts inferred parents & Official Pango parents & n & Other descendants \\
Pango &  &  &  &  \\
\midrule
