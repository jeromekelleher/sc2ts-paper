...
Early in the SARS-CoV-2 pandemic, the lack of genetic variation
meant that recombination had little effect.
...
recombinant strains is a time-consuming, often manual process [Citation?].
...
More recently, \cite{Turakhia2022-it} presented a
...
Wohns2022-th}, and the supporting \texttt{tskit} software library is now widely
...
based on their collection dates, and proceeds in three phases. Firstly,
likely attachment nodes in the current ARG
(or paths, if inferred to be a recombinant) are
chosen for each of the samples in the daily batch using the
Li and Stephen's model (Section~\ref{sec:ls}). Then, since many samples
in a daily batch can share an attachment path,
we infer trees for each of the clusters separately using standard
...
and mutational spectra (Section~\ref{sec:mutation_spectrum})
while simultaneously identifying the majority of known recombinant
