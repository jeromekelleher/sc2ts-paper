...
starts with an ``X''. We primarily focus on the Long ARG in this section
since it contains many more recombinants, briefly summarising the status
of Pango X lineages for the Wide ARG in Section~\ref{sec-pango_x_wide_arg}.

When using Nextclade Pango lineage designations, the
Long ARG contains 749 samples from 33 recombinant lineages; this
reduces toe 711 and XX when singleton recombinants (i.e., recombination
nodes subtending only one sample) are removed.
In contrast, when using GISAID Pango lineage assignments (for the same
data) we have 515 samples from 38 Pango recombinant lineages,
indicating that there is substantial uncertainty in the assignment of
Pango X lineages to samples.

Of the 749 Nextclade-designated recombinant samples, all but two descend
from a recombination node in the Long ARG.
