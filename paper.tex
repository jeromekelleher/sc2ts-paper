...
exclude ``singleton'' recombination nodes (those ancestral to only one sample).
Figure~\ref{fig:recomb_mrcas}A plots breakpoints for all nonsingleton recombinants,
and Figure~\ref{fig:recomb_mrcas}B plots a subset of potentially more
reliable breakpoints that occur in nodes
ancestral to 10 or more samples (note that this is one of the official criteria
used when proposing Pango recombinant lineages [CITE]).
...
that are only a matter of weeks apart.
...
starts with an ``X''. As the Wide ARG was only sampled until mid 2021,
the majority of these Pango X lineages are found only in the Long ARG.
...
Nextclade-designated Pango recombinant lineages comprising 749 samples
(711 when singleton recombinants are removed). In
...
the maximum genomic position for the recombination breakpoint, here at 22227 basepairs
(corresponding to a breakpoint strictly less than 22228, see Table~\ref{tab:jackson})
...
suggesting that most of the simple multiple origin examples are due to Nextclade mislabelling.
...
The Long ARG subgraph is plotted in Figure~\ref{fig:complex_origins_graph}, showing a recombination
...
pattern is seen when examining XB in the Wide ARG (see below).
...
27389bp) and B.1.36.8 (the remaining fragment of the genome). Above, we noted that
in the long ARG the equivalent recombination node has only 2 parents,
