...
The lack of genetic variation made recombination challenging to detect early in the SARS-CoV-2 pandemic. In late 2020, the appearance and
...
encoding of large ARGs. This representation should be well-suited to storing
large recombinant viral genealogies such as SARS-CoV-2,
which currently encompasses over 15 million sequences in the GISAID database
\citep{Shu2017-hp}). Moreover the \texttt{tskit} software library, which implements this
...
(\texttt{sc2ts}, optionally pronounced ``scoots'') that builds on the advances above
...
we demonstrate that \texttt{sc2ts} can be used to simultaneously (1) detect putative SARS-CoV-2
...
\texttt{tskit} (tree sequence toolkit) software library \citep{Kelleher2018-xc}.
\texttt{Sc2ts} is a promising method to automate detection of SARS-CoV-2 recombinants
...
Our \texttt{sc2ts} backbone phylogenies do contain small amounts of recombination:
there are 7 recombination nodes in the backbone of the Wide ARG and 8 in the backbone
of the Long ARG. However, the backbones remain extremely tree-like, as these
recombination events involve minor, local topological rearrangements. In particular,
recombination in the backbone phylogenies only occurs between close relatives
(e.g.\ sibling lineages), sometimes even within a single lineage such that the parent
lineages above a recombination event coalesce with each other before involvement with
any other part of the phylogeny. These local reticulations could either represent
real but previously unobserved recombination events between closely related strains,
or could be artifacts of the current \texttt{sc2ts} method and its parameterization.
...
\cmidrule{3-5}
B            & 2   & Jackson        &  6,529--6,953 & B.1.36/B.1.1.7  \\
...
Intervals are given in left-inclusive, right-exclusive form.
...
are similar there is no way to distinguish them. Here we define the
interval for a particular breakpoint as the interval over which the
sequences for the left and right parent nodes are identical. The right-hand
extreme of the interval is chosen by the LS HMM Viterbi algorithm
(Section~\ref{sec:ls}, and the left bound can then be derived by
sequence comparison of the parents.
...
breakpoints, as a function of parental divergence. MRCAs of parents
inferred to have caused a Pango designated recombinant (XA, XB, etc.)
identified in orange (A) 851 parent pairs (singleton recombinants removed).
Parental strains often descend from the same MRCA; each horizontal dotted line
indicates a frequently found MRCA: these tend to be associated with major waves
and often have many immediate children (i.e. are large
polytomies, likely to indicate undersampled outbreaks) (B) Further filtered
to 358 parent pairs by removing HMM inconsistent breakpoints. For equivalent
plots broken down by parental VoC classification, see
Figure~\ref{fig:recomb_mrcas_voc_breakdown}
}
\end{figure}

Although recombination nodes are relatively infrequent, there are still many
present in the wide and long ARGs (4123 and 2078 respectively; Supplementary
Tables 1 and 2). Each recombination node was inferred as a
result of matching a specific (``causal'') recombinant sample into the ARG.
However, ~63\% of them are ancestral to only a single
...
majority of newly identified recombinants arise from parents that diverged 10 to 30 weeks previously.
However, there is also a peak of recombinants that diverged ${\sim}95$ weeks prior to the event:
these correspond to recombinants whose parental common ancestor traces to early 2020,
including both Delta-Delta and Delta-Omicron recombinants
(Supplementary Figure~\ref{fig:recomb_mrcas_voc_breakdown}).
Note that each plotted point represents a breakpoint, so that a recombination
event with more than one breakpoint (e.g. with 3 or more parents, comprising
${\sim}10 \%$ of the recombination events in Figure ~\ref{fig:recomb_mrcas}A and
${\sim}3 \%$ in Figure ~\ref{fig:recomb_mrcas}B) will be represented by several points.

As a check on the reliability of these identified recombinations,
Figure~\ref{fig:recomb_mrcas}B  plots only breakpoints which are
HMM-consistent. This reduces both the number of recombination nodes \&
breakpoints in the long ARG from 763 \& 851 to 347 \& 358
(and in the wide ARG from 1522 \& 1769 to XXX \& YYY , see
...
figure. These shared MRCAs lie near the root of large expansions of certain
lineages (B.1: original variant, B.1.617.2: Delta, BA.1: first Omicron wave,
and BA.2: second Omicron wave), as evidenced by the number of samples with those
...
are associated with extensive polytomies in the tree (at the extreme,
the BA.2-linked node has 2,872 immediate children). This is likely to
indicate a rapid and under-sampled expansion of a clade in the SARS-CoV-2 genealogy.

Finally, it is clear from Supplementary Figure\ref{fig:recomb_mrcas_voc_breakdown} that of the recombinant
...
XXX) comprising 749 samples (711 when singleton recombinants are removed). In
...
The remaining remaining 630 Pango recombinant samples, representing 31
Nextclade-designated Pango lineages, trace back to 50 different most recent
recombination nodes.
We examine the inferred inheritance structure in the following subsections.
...
\caption{\label{fig:pango-simple-origin-graph} Simple origin
...
imputed Pango status. Genomic regions inherited by the recombination node use 
breakpoint positions in the ARG, corresponding to the maximum inferred breakpoint position
...
A simple example is the case of XA, illustrated in Figure~\ref{fig:pango-simple-origin-graph}A.
This shows a subgraph of the long ARG, in which paths have been traced from all
...
that the subgraph is not extended to show other descendants of recombinant parents. It is clear
from the subgraph that all the XA
samples trace to a single originating recombination node,
...
all trace back to the same most recent common recombination node 
...
In total out of the 31 Pango recombinant lineages in the long ARG we identify 16 as
having a single origin. Supplementary Table~\ref{tab:pango-single-origin} summarises these
together with their inferred parents, showing good agreement between \texttt{sc2ts} and
the official Pango designated parents, with \texttt{sc2ts} providing more a precise
parent designation. Six of the 16 lineages (XA, XAC, XAE, XF, XK, and XS) are of the
simple-origin (XA) type, whereas the rest are of the XAG type, and include multiple Pango
designated lineages as descendants of the originating recombination event. In some cases,
...
a single recombination event. In this section we examine several Pango X lineage
...
XE lineage (originally defined on the basis of sequences from the UK and Ireland). The
Long ARG infers five independent origins for nonsingleton Nextclade-designated XE
samples (Supplementary Table~\ref{tab:multiple_origins_table}). One of the recombinant origins,
caused in our ARG by an English sample, accounts for 155 out of 164 XE samples
(primarily UK and Irish, but with a few northern European and north American strains).
This certainly represents the XE recombinant lineage as originally defined. The other recombination nodes
only give rise for one, two, or three XE samples. Three of these recombination
nodes are likely to represent Nextclade mislabelling: they either have different
inferred breakpoint positions or (in the case of the earliest recombination node),
descendants that are primarily labelled as other Pango X-lineages. Additional support
for the suggestion that they result from incorrect labelling comes from the fact that
none of their XE-labelled descendant samples are from the UK or Ireland.
It is less clear if the most recent XE cluster is a mislabelling or a case of \texttt{sc2ts}
inferring multiple recombinations when there should be just one. This cluster of 2 samples
(one of which is English) has exactly the same breakpoint estimates as the
main cluster. If the only samples available are rather genetically distant from an originating
...
such as the examples in Figure~\ref{fig:pango-simple-origin-graph}, four of the
Pango X lineages (XB, XE, XAB, and XAJ) have Nextclade-designated samples which
are related in a more complex manner, where multiple non-independent (``nested'')
recombination events occur. An example is XB in the long ARG,
...
be a fruitful avenue for future work. Examples include a stochastic traceback through the collection of paths at the global optimum to glean further information about our the likelihood surface, and determine whether there are downstream implications for our conclusions. Here, we have considered the Viterbi algorithm to make statements about the most likely paths through the data. The machinery used here can be modified to run the forwards and backwards algorithms which determine the probability of observing a focal sequence, integrating over all possible paths through the data, under LS model [cite D-Palms]. This presents an opportunity to estimate parameters of interest while exploiting the tree-structure to gain log based speed-ups in exact their exact implementation.
...
\toprule
Focal Pango  & Num origins & Num focal samples (further split by origin, *=nested) & Official Pango parents & \textbf{Main clade}: sc2ts parents & \textbf{Main clade}: other descendants \\
\midrule
...
Summary of all Pango X-lineages in the long ARG. In cases of multiple origins, most X-lineages have a single ``main clade'' (in bold). 
...
\begin{sidewaystable}
\centering
\begin{tabular}{llp{1.2cm}llrl}
\toprule
Causal date & Sc2ts inferred parents & Causal lineage & Causal strain & Breakpoint & $n_{XE}$ & Other descendants \\
\midrule
2022-01-06 & BA.1.20 + BA.2.9 & XH & Denmark/DCGC-323778/2022 & (10448,11538) & 3 & XAF: 34/35, XH: 11/11, B.1.1.529: 2 \\
2022-02-11 & BA.1.17.2 + BA.2 & XE & England/MILK-3674F1A/2022 & (10448,11538) & 155 &  \\
2022-02-17 & Unknown + BA.2 & XE & Turkey/HSGM-G2220/2022 & (17411,19956) & 1 & B.1.1.529: 1 \\
2022-04-09 & BA.1 + BA.2 & XE & Belgium/ULG-25782/2022 & (9867,10199) & 3 &  \\
2022-04-20 & BA.1.1.15 + Unknown & XE & Canada/BC-BCCDC-424071/2022 & (10448,11538) & 2 &  \\
\bottomrule
\end{tabular}\caption{\label{tab:multiple_origins_table}
Summary of inferred multiple ``origins'' for Nextclade-designated XE samples
(Official: BA.1* \& BA.2, 10448–11287, defined from UK and Irish strains, earliest
on 2022/1/16;  see \protect\url{https://github.com/cov-lineages/pango-designation/issues/454}).
XE samples are grouped by their most recent ancestral recombination node;
Details are also provided for the ``causal strain'' that led to the recombination
being proposed by \texttt{sc2ts}.
}
\end{sidewaystable}
