...
Emissions may also be a function of the nucleotide states, but in our RNA virus case, we assume that \sally{mutations occur to all possible alternate alleles} ($A,C,G,U,-$) with equal probability for a mutation to any other allele (including a gap in the alignment; $-$) are equally likely $\mu_\ell/4$. This is reasonable for rapidly evolving pathogens, but we note that setting the number of alleles at site $\ell$ ($a_\ell$) to the set of observed alleles across all analysed samples ($\mu_\ell/(a_\ell-1)$) will often be more appropriate
...
where $\alpha = (1 - r) + r/n$ represents the probability of a sequence not recombining at a given position with any of the other $n-1$ members of the reference panel (Figure~\ref{fig:ls_diagram}, red panel) and $c$ is the
...
probabilities. Another, which we carry out within \texttt{sc2ts}, is to assume sites with missing nucleotides are uninformative to the path probability, and impute them as each focal sequence is analysed, on the fly. We achieve this by setting the emission probability from any state $(A,C,G,U,-)$ to `unknown' as 1. Then, after determining a most likely copying path, we can impute these missing data by simply copying nucleotides present on the most likely copying path at the sites of missing data in the focal sequence. Note that by iteratively imputing missing data in focal sequences in this way, the reference panel being used for any newly observed focal sequence is guaranteed to be devoid of missingness.
...
this node (in gold) is a large polytomy with
107 immediate children, and is the ancestor of the majority of Delta samples.
