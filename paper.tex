...
all involve recombination between closely related lineages (e.g.\ sibling lineages)
and a number even represent recombination within a single lineage, in which the parent
lineages above a recombination event coalesce with each other before coalescing
with any other lineages in the phylogeny. These could either represent true
recombination events between closely related strains, which have previously gone unnoticed,
...
Although coronaviruses are generally known to recombine [CITATION?]
it was not clear that SARS-CoV-2 recombinants with ongoing transmission
existed early in the pandemic.
\citet{Jackson2021-ik} performed a careful analysis of sequences
...
The remaining remaining 630 Pango recombinant samples trace back to 50
...
Nextclade-identified XA samples to the closest other sample strains in our dataset.
Dotted lines show where this subgraph links to other nodes within the ARG, and above
recombination nodes only ancestral nodes are shown (i.e. we do not descent to include
cousins of recombinant parents).
It is clear from the subgraph that all the XA samples trace to a single originating
...
A more complex single-origin case is Figure~\ref{fig:pango-simple-origin-graph}B shows a case of another
...
Supplementary Table~\ref{tab:pango-single-origin} summarises the sequences of all 16 of the
