...
[JK: TODO, expand this next paragraph to two, explaining a bit more that
we're searching among every node in the ARG, using the efficient tree
implementation in tsinfer, and a few words about how we can use the
HMM more, to do things like stochastic traceback, etc.]
...
Single origin Nextclade Pango X lineages. (A) Subgraph for XA in the long ARG: all XA samples and their ancestral lineages are shown outwards to the nearest sampled viral genome; dotted lines show where the ARG continues. Nodes are coloured by Nextclade Pango designation; smaller symbols are nodes inserted by  sc2ts (i.e. not samples); their Pango designation has been imputed where possible. (B) Details for the single-origin Pango X lineages in the long ARG. (C) XAG origins in the long ARG.}
\end{figure}
...
In Figure XXX we give the details of these multiple pango lineages, along with an example lineage subgraph. As with the subgraphs in Figure~\ref{tab:pango-single-origin}, the equivalent XD plot with GISAID Pango designations, along with identifying labels, is shown for comparison in Supplementary Figure XXX
...
nested recombination events; moreover, this sample is not identified as XB by GISAID (Supplementary Figure~\ref{}). We suggest that investigating cases of complex origins, and identifing which (if any) of the nested, further recombination events may be artifactual, is an important area of future research.
