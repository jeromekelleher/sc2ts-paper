...
% (e.g., \url{https://pha4ge.org/resource/identifying-sars-cov-2-recombinants/}).
...
ancestral state is the gap state ``-'' and the derived state is a
nucleotide, and vice versa for deletions. A recurrent mutation is
one that occurs at a site which has previously mutated (i.e. where one
or more mutations at that site occur on the edges ancestral to the mutation),
...
is large across each ARG (Wide ARG mean: 41.23, median: XXX;  Long ARG mean: 36.10, median: YYY; Table~\ref{tab:args}). This is
...
been central to pandemic responses thus
