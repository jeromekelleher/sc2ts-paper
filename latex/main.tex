...
Among the 44 Pango X lineages present in the ARG,
16 are fully concordant with the expected hierarchical structure 
and have strong agreement
with the community-consensus parent lineages and breakpoint intervals
(similar to specialised detection methods; 
see Methods, Table~\ref{tab:method_concordance}).
The early recombinants identified by Jackson et al.\cite{Jackson2021}
(including XA; Figure~\ref{fig:subgraphs}A)
are also captured with high fidelity in the ARG 
...
Using the inferred ancestral recombination graph (ARG), we examined the origins
and relationships of epidemiologically relevant SARS-CoV-2 recombinant lineages
designated in the Pango nomenclature. Among the Pango X lineages present in the
dataset, we identified a set whose oldest assigned node corresponds to a
recombination event in the ARG and is ancestral to all samples assigned to that
lineage. These lineages therefore have fully concordant recombinant origins in
the ARG, with inferred parental lineages and breakpoint intervals closely
matching previous reports. XA provides a representative example of this pattern
(Fig. 2A).

We also identified several cases in which multiple named recombinant lineages
descend from a shared ancestral recombination event. In these instances, the
ARG shows that the origins of the lineages are nested within a single
recombinant ancestry, rather than corresponding to independent recombination
events (Fig. 2B). This structure is captured directly by the
recombination-aware genealogy and is not apparent from tree-based phylogenies
inferred without modelling recombination.

A smaller number of designated recombinant lineages show limited or ambiguous
support for recombination in the ARG. Some of these lineages differ from a
single parental lineage by only a small number of mutations, while others are
consistent with recombination but lack sufficient representation of parental
diversity to reconstruct the event confidently. In addition, several complex
recombinants with multiple breakpoints were excluded under the conservative
inclusion thresholds used here. Detailed results for all evaluated Pango X
lineages are summarised in the Supplement.
