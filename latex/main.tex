...
% Using line numbers for initial submission
\usepackage{lineno}
...
\usepackage{amssymb}
\usepackage{setspace}
\ifdefined\SPLITBUILD
\usepackage[draft]{hyperref}
\else
\usepackage[hypertexnames=false,hidelinks]{hyperref}
\fi

\newcommand{\supref}[1]{SI~\S\ref{#1}}
...
\affil[$\ast$]{\small{Corresponding author: shing.zhan@ndph.ox.ac.uk (S.H.Z.); jerome.kelleher@bdi.ox.ac.uk (J.K.)}}
...
The COVID-19 pandemic generated an unprecedented volume of viral genomic data,
enabling real-time tracking of pathogen evolution at global scale.
Genealogical reconstructions provide the primary framework for interpreting
such data and underpin many downstream analyses, including lineage designation,
phylodynamics, and molecular epidemiology. Most approaches to inferring
genealogies from viral sequence data, however, assume that recombination is
negligible and that ancestry can be represented as a single
tree~\cite{Rannala2025recombination}.
While this assumption is often reasonable for short timescales, recombinant SARS-CoV-2
lineages have risen to global prevalence\cite{Roemer2023,Shiraz2023enhanced},
rendering purely tree-based representations incomplete and potentially misleading.

The scale and tempo of SARS-CoV-2 genomic surveillance posed fundamental
challenges to existing phylogenetic approaches. At the peak of the pandemic,
tens of thousands of genomes were generated daily, precluding repeated de novo
phylogenetic inference. The UShER method~\cite{Turakhia2021} enabled real-time
reconstruction of a comprehensive genealogy by rapidly placing new samples into
an existing tree and proved indispensable for genomic surveillance and lineage
tracking\cite{Hinrichs2024}.
However, as with other tree-based methods, UShER must force
recombinant genomes into a single parental position, distorting genealogical
relationships and requiring recombination to be identified and interpreted post
hoc. Although numerous methods have been developed to detect recombinant
genomes~\cite{Varabyou2021rapid,%Bolotie
Turakhia2022,%RIPPLES
Zhou2023virusrecom,
Alfonsi2024,%RecombinHunt
Li2024CovRecomb},
this information is rarely incorporated directly into the phylogenies
used for downstream analyses~\cite{McBroome2021,Hadfield2018},
leaving a disconnect between recombination
detection and genealogical inference.
...
for several decades\cite{Hudson1983,Griffiths1997ancestral},
recent breakthroughs in
simulation
and inference
\cite{Rasmussen2014,Kelleher2016,Kelleher2018efficient,Kelleher2019,Speidel2019}
have led to a surge
...
recombination \cite{Hudson1983,Griffiths1997ancestral} to a
rich data structure representing recombinant
ancestry\cite{Minichiello2006mapping,Wong2024}.

Here we present sc2ts, a new method to infer ARGs for pathogens like SARS-CoV-2
...
collection of SARS-CoV-2 genomes purged of various systematic errors
and artifacts~\cite{Hunt2024},
we present an ARG with 2.48 million high-quality genomes.
We show that the non-recombinant portions of this ARG are
congruent with the UShER phylogeny built
...
known recombinant lineages that were painstakingly collated through
a community effort and provides novel insights into the relationships between these
...
while allowing recombination ($k = 4$) or strongly penalising
against it ($k = 1000$).
}
...
recombination rates varying along the genome (Methods).
...
For example, a tabular summary of the 2.75 million nodes
including sample IDs and Pango assignments
can be derived in less than half a second.
...
accurately depicting a range of results from the literature.
...
(\supref{sec:supplement_sc2ts_usher_parsimony})
and imputation of missing and ambiguous bases
(\supref{sec:supplement_sc2ts_usher_imputation}).
...
(\supref{sec:supplement_pango_lineages}).
...
\supref{sec:evaluating_node_dates}).
Finally, we verified
that the origins of the
Alpha (Figure~\ref{fig:subgraph_alpha}, \supref{sec:dd_alpha}),
Delta (Figure~\ref{fig:subgraph_delta}, \supref{sec:dd_delta})
and
Omicron (Figure~\ref{fig:subgraph_omicron}, \supref{sec:dd_omicron})
...
See \supref{sec:supplement_dels} for further analysis.
...
For more detailed visualisations of these subgraphs, see Document S1.
}
\label{fig:subgraphs}
\end{figure}

Although the Pango nomenclature system\cite{Rambaut2020} provides clear
rules for designating new lineages, the identification of
recombinant (``X'') lineages requires the manual synthesis
of multiple lines of evidence\cite{Smith2023tracking}.
By automatically detecting and incorporating recombination events into the ARG,
sc2ts provides a systematic framework for evaluating these origins.
Of the 44 Pango X lineages present in the dataset
(\supref{sec:supplement_x_pango_lineages};
Table~\ref{tab:pango_x_lineages}),
16 map precisely to
recombination nodes in the ARG that are ancestral to all samples assigned to
the lineage, indicating fully concordant recombinant origins.
For these
lineages, the inferred parental lineages and breakpoint intervals show strong
agreement with community consensus and with results from specialised
recombination detection methods
(Methods, Table~\ref{tab:method_concordance}).
Early well-characterised recombinants\cite{Jackson2021} are also captured
with high fidelity in the ARG
(\supref{tab:jackson}),
and XA provides a representative example (Figure~\ref{fig:subgraphs}A).
In addition, several complex recombinants with multiple
breakpoints were excluded from the ARG under the conservative thresholds used
here but were nonetheless detected by the HMM
(\supref{sec:supplement_rejected_pango_x_lineages};
Table~\ref{tab:extra_hmm_results}).

In contrast, the ARG indicates that multiple named recombinant lineages descend
from shared ancestral recombination events rather than representing independent
origins.
Specifically, 14 Pango X lineages are close descendants of 7 distinct
recombination events, with up to 5 lineages descending from a
single recombination event (Figure~\ref{fig:subgraphs}B).
The remaining designated recombinant lineages show limited or ambiguous support for
recombination in the ARG. In some cases, the lineage origin nodes differ from the
parent by a small number of mutations (Figure~\ref{fig:subgraphs}C),
while in others
the reconstruction is constrained by the absence of sufficient parental
diversity in the dataset. Extended analyses of all Pango X lineages, including
supporting evidence and alternative interpretations, are provided in
\supref{sec:supplement_indepth_pango_x_lineages},
along with copying patterns (Figure~\ref{fig:pango_x_copying_patterns})
and detailed ARG visualisations (Document S1).

\subsection*{Quality control and validation of recombination events}
Several different error modalities can cause short regions
a sample genome to match more closely
to an alternative genome rather than the true parent,
which can be mistaken for evidence of recombination.
...
which effectively identifies a range of issues.
...
those identified by Jackson et al.\ (\supref{sec:jackson_recombs}).
...
The sc2ts inference framework provides a quantitative measure of support for
each inferred recombination event by comparing the most parsimonious copying
path that allows recombination with an alternative placement that disallows
recombination (Figure~\ref{fig:methodology}).
The difference between these two solutions, measured as the
number of mutations ``averted'' by allowing recombination, serves as an index of
the evidence supporting a recombinant origin.
Across the recombination events
passing quality-control filters, support spans a wide range, from marginal
cases with only a small number of averted mutations to strongly supported
events for which a non-recombinant placement would require dozens of additional
mutations. For example, the most parsimonious non-recombinant placement of XC
would require 37 additional mutations, indicating strong support for
recombination (Figure~\ref{fig:recombinant_evidence}A;
Table~\ref{tab:pango_x_lineages}).
Strongly supported events show high concordance with independent validation by
3SEQ, whereas the 5\% of events that fail 3SEQ validation are
concentrated among the weakest-supported cases ($\leq 5$ averted mutations)
and associated with recently diverged parents
(Figure~\ref{fig:recombinant_evidence}B,C).
...
recombination nodes associated with Pango X lineages
(Table~\ref{tab:pango_x_lineages})
are labelled starting with `X' and
...
the events associated with the Jackson et al.\ recombinants
span 5 to 48 days.
...
relative to the global case numbers (left y-axis).
...
(parents must precede recombinant offspring)
and the differing sets of problematic sites masked
by sc2ts and UShER
(\supref{sec:detailed_ripples}).
...
(\supref{sec:detailed_ripples}).
Thus, a more lenient threshold ($p=3$) was needed for UShER+RIPPLES to detect a
similar fraction of Pango X lineages as sct2ts with $k=4$, resulting in more
false positives. That said, XB and XAJ are among the lineages detected by
UShER+RIPPLES, even at $p=4$, strengthening evidence that these are false
negatives for sc2ts (\supref{sec:dd_xaj}).
...
infer the ARG, sc2ts.
The ability to build an ARG in real time from millions of whole
...
% to other pathogens.
The LS HMM is the most important element of
...
efficient
algorithms~\cite{Kelleher2016,Ralph2020,Kelleher2020coalescent,
Grundler2025geographic,Lehmann2025args}.
The mature and feature-rich tskit library has interfaces in Python, C and Rust,
extensive documentation,
and underpins a rapidly growing software ecosystem of
simulation~\cite{Baumdicker2022,Haller2019,Petr2023,Tsambos2023link,Tagami2024tstrait,Gopalan2025bridging},
...
and data
analysis~\cite{Fan2022,Nowbandegani2023extremely,Fritze2024forest,Lee2025genetic}
tools.
...
GiB TSV file) to VCF Zarr using
pyfaidx~\cite{Shirley2015efficient} and Pandas~\cite{Mckinney2010data} in a
...
Supplementary text~\ref{sec:supplement_sc2ts_usher_imputation} gives an example
...
were also omitted,
as the number of samples on this date ($\sim$40,000) was a major outlier
...
Although these sites are recommended to be masked for phylogenetic
inference~\cite{Turakhia2020stability},
...
# https://github.com/jeromekelleher/sc2ts-paper/issues/1004
To do so, we first built a preliminary ARG
from the samples collected up to June 30, 2021,
without any site masking.
Then, we identified the sites with at least 200 mutations in the ARG.
Among these 120 highly mutated sites,
we excluded those having lineage-defining mutations
(as taken from a global UShER phylogeny built using GISAID data;
\url{https://github.com/andersen-lab/Freyja/blob/main/freyja/data/lineage_mutations.json},
commit edda27c),
leaving a set of 11 sites.
# https://github.com/jeromekelleher/sc2ts-paper/issues/1004#issuecomment-4262825159
Six of these sites were present in the problematic sites file
(\url{https://github.com/W-L/ProblematicSites_SARS-CoV2}).
In addition, we masked two more problematic sites
where sequencing errors frequently occur with the ARTIC V4 primers~\cite{Sanderson2021issues}.
We therefore masked 13 sites
to reduce the effect of sequencing errors and homoplasy
on the matching performance of the LS HMM,
without impacting Pango lineage assignment later on.
...
using Numba accelerated \cite{Lam2015numba} Python code.
...
and is widely used in human genomics~\cite{Delaneau2019-wl,Browning2018-nk}.
...
``saltations'' associated with major VOCs.
...
``retrospective sample group'' using the same mechanisms
...
for further explanation, see \supref{sec:dd_delta}.
...
For further explanation, see \supref{sec:dd_omicron}.
...
(Figures~\ref{fig:inference_resources},\ref{fig:samples_per_day}) with all
...
we chose 75 sites for deletion-remapping (see \supref{sec:supplement_dels} for analysis of the major deletions identified from this process).
...
version 0.2.3. See \supref{sec:evaluating_node_dates} for
...
\subsubsection*{Visualising recombinant copying patterns}
...
Figure~\ref{fig:pango_x_copying_patterns}
shows copying patterns for Pango X lineages in the ARG,
...
(see \supref{sec:dd_xbb} for more details).
See also Figure~\ref{fig:copying_pattern_examples} for copying patterns
...
we treated the entire cluster of sites as a single ``locus''.
...
We validated the conversion by comparing the
...
\supref{sec:spurious-ancestrally-important-recombinants},
...
not available for this pipeline (see \supref{sec:detailed_ripples}
...
\[
I_{left} = \frac{L}{n T \mu} \frac{\Gamma(1+n) - \Gamma(1+n, P T \mu)}{
\Gamma(n) - \Gamma(n, P T \mu)},
\]
...
Specifically, for each RIPPLES event identified by a list of
...
pipeline.
See \supref{sec:detailed_ripples} for detailed analysis
...
\item The sequence alignments in VCF Zarr format have been deposited at
...
see \supref{sec:supplement_dels}).
...
\supref{sec:dd_xm} for further details.
...
See \supref{sec:dd_xx} for more details on the Xx event.

In cases where nested samples not assigned to the focal Pango lineage,
these are
...
the 44 Pango X lineages present in the ARG.
...
than de novo recombinant lineages (\supref{sec:dd_xq}).
...
(Figure~\ref{fig:subgraphs}B, \supref{sec:dd_xx}).
...
to suggest recombination (\supref{sec:dd_xn}).
...
parent lineages were not present (\supref{sec:dd_xb}).
...
\supref{sec:supplement_rejected_pango_x_lineages} and Table
\ref{tab:extra_hmm_results} for details.
See \supref{sec:supplement_indepth_pango_x_lineages}
...
and Document S1 for ARG visualisations of the Pango X lineages.
...
the subgraph illustrations in Document S1.
...
See also Document S2 for exact positions and nucleotide bases.}
